/*
* GPU filter example, the RAW variant: the same bitwise invert as gpu_invert_example.c but
* WITHOUT the execution pool -- every obligation the pool normally discharges is spelled out
* by hand here. Read the pooled version first; it is the recommended shape. This one exists
* for filters that need what the pool does not model -- their own queues, foreign-API
* submissions, custom pacing -- and as documentation of what gpuExecAcquire/Submit actually
* do on a filter's behalf:
*
...
static void VS_CC invertCreate(const VSMap *in, VSMap *out, void *userData, VSCore *core, const VSAPI *vsapi) {
    ...
    if (d->vk->vkCreateComputePipelines(d->h.device, VK_NULL_HANDLE, 1, &pipeInfo, NULL, &d->pipeline) != VK_SUCCESS) {
        vsapi->mapSetError(out, "InvertRawGPU: pipeline creation failed");
        goto fail;
    }
...

    {
        ...
        vsapi->createVideoFilterEx(out, "InvertRawGPU", &d->vi, invertGetFrame, invertFree, fmParallel, ffGPUOutput, deps, 1, d, core);
    }
...
VS_EXTERNAL_API(void) VapourSynthPluginInit2(VSPlugin *plugin, const VSPLUGINAPI *vspapi) {
    vspapi->configPlugin("com.example.gpuinvertraw", "vkrawexample", "Out of tree GPU filter example, raw submission variant", VS_MAKE_VERSION(1, 0), VAPOURSYNTH_API_VERSION, 0, plugin);
    vspapi->registerFunction("InvertRawGPU", "clip:vnode:gpu;", "clip:vnode:gpu;", invertCreate, NULL, plugin);
}
