...
class Dependency(object):
    ...
    def add_to_global(self):
        if 'global' in self.attributes:
            if self.attributes['global'] == True:
                env = self.ctx.env
                env['GLOBALDEPS'] = env['GLOBALDEPS'] + [self.identifier]
    ...
    def add_to_groups(self):
        if 'groups' in self.attributes:
            ...
            for g in groups:
                gname = 'DEPGROUP_' + DependencyInflector(g).storage_key()
                env[gname] = env[gname] + [self.identifier]

    def add_own_dependencies(self):
        key = DependencyInflector(self.identifier).storage_key()
        self.ctx.env["DEPS_" + key] = self.own_deps

        for dep in self.own_deps:
            self.ctx.env["DEPS_" + key] += self.ctx.env['DEPS_' + dep]
    ...
    def check_any_dependencies(self):
        if 'deps_any' in self.attributes:
            ...
            if len(deps & self.satisfied_deps) == 0:
                ...
                raise DependencyError
            self.own_deps += (deps & self.satisfied_deps)
...
def group_dependencies(ctx, group):
    return ctx.env['DEPGROUP_' + DependencyInflector(group).storage_key()]

def dependencies_use(ctx, dep):
    return ctx.env['DEPS_' + DependencyInflector(dep).storage_key()]

def dependencies_libs(ctx, dep):
    libs = []

    for d in dependencies_use(ctx, dep):
        libs += ctx.env['LIB_' + DependencyInflector(d).storage_key()]

    return libs

def dependencies_includes(ctx, dep):
    includes = []

    for d in dependencies_use(ctx, dep):
        includes += ctx.env['INCLUDES_' + DependencyInflector(d).storage_key()]

    return includes

def dependencies_global(ctx):
    return ctx.env['GLOBALDEPS']

BuildContext.filtered_sources = filtered_sources
BuildContext.group_dependencies = group_dependencies
BuildContext.dependencies_use = dependencies_use
BuildContext.dependencies_libs = dependencies_libs
BuildContext.dependencies_includes  = dependencies_includes
BuildContext.dependencies_global = dependencies_global
