...
public:
    ...
    bool __stdcall GetParity(int n) { return true; }  //fixme?
    void __stdcall GetAudio(void* buf, int64_t start, int64_t count, IScriptEnvironment* env) {}
    void __stdcall SetCacheHints(int cachehints, int frame_range) { /*ignore*/ }
    const VideoInfo &__stdcall GetVideoInfo() { return vi; }
    ~VSClip() { vsapi->freeNode(clip); }
};
...
struct WrappedClip {
    ...
    ~WrappedClip() { clip = NULL; delete fakeEnv; }
};
