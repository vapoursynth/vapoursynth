...
public:
  ...
  const uint8_t* GetReadPtr() const { return data; }
  uint8_t* GetWritePtr() { ++sequence_number; return data; }
...
class VideoFrame {
  ...
  void Release() { if (refcount==1) InterlockedDecrement(&vfb->refcount); InterlockedDecrement(&refcount); }
...
public:
  ...
  const uint8_t* GetReadPtr() const { return vfb->GetReadPtr() + offset; }
  const uint8_t* GetReadPtr(int plane) const { return vfb->GetReadPtr() + GetOffset(plane); }
...
public:
  ...
  PClip AsClip() const { Q_ASSERT(IsClip()); return IsClip()?clip:0; }
  bool AsBool() const { Q_ASSERT(IsBool()); return boolean; }
  int AsInt() const { Q_ASSERT(IsInt()); return integer; }
  const char* AsString() const { Q_ASSERT(IsString()); return IsString()?string:0; }
  double AsFloat() const { Q_ASSERT(IsFloat()); return IsInt()?integer:floating_pt; }

  bool AsBool(bool def) const { Q_ASSERT(IsBool()||!Defined()); return IsBool() ? boolean : def; }
  int AsInt(int def) const { Q_ASSERT(IsInt()||!Defined()); return IsInt() ? integer : def; }
  double AsFloat(double def) const { Q_ASSERT(IsFloat()||!Defined()); return IsInt() ? integer : type=='f' ? floating_pt : def; }
  const char* AsString(const char* def) const { Q_ASSERT(IsString()||!Defined()); return IsString() ? string : def; }

  int ArraySize() const { Q_ASSERT(IsArray()); return IsArray()?array_size:1; }
