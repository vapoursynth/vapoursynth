...
typedef struct {
    ...
    double amplitude;
    double frequency;
    enum WaveForm {Constant, Square, Triangle, SawTooth, Sine} waveform;
} WaveGeneratorData;

static const VSFrame* VS_CC waveGeneratorGetframe(int n, int activationReason, void* instanceData, void** frameData, VSFrameContext* frameCtx, VSCore* core, const VSAPI* vsapi) {
    ...
    if (activationReason == arInitial) {
        frame = nullptr;
            ...
            const uint8_t *dst = vsapi->getReadPtr(frame, 0);
            for (int channel = 1; channel < d->ai.format.numChannels; channel++)
                memcpy(vsapi->getWritePtr(frame, channel), vsapi->getReadPtr(frame, 0), samples * d->ai.format.bytesPerSample);
...
static void VS_CC waveGeneratorFree(void* instanceData, VSCore* core, const VSAPI* vsapi) {
    BlankAudioData* d = reinterpret_cast<BlankAudioData*>(instanceData);
    vsapi->freeFrame(d->f);
    delete d;
}

static void VS_CC waveGeneratorCreate(const VSMap* in, VSMap* out, void* userData, VSCore* core, const VSAPI* vsapi) {
    ...
    const char* filterName = (userData ? "BlankClip" : "WaveGenerator");
    ...
    if (d->ai.sampleRate <= 0)
        RETERROR((std::string(filterName) + ": invalid sample rate").c_str());

    if (d->ai.numSamples <= 0)
        RETERROR((std::string(filterName) + ": invalid length").c_str());

    if (!vsapi->queryAudioFormat(&d->ai.format, d->ai.format.sampleType, d->ai.format.bitsPerSample, d->ai.format.channelLayout, core))
        RETERROR((std::string(filterName) + ": invalid format").c_str());

    vsapi->createAudioFilter(out, filterName, &d->ai, blankAudioGetframe, blankAudioFree, d->keep ? fmUnordered : fmParallel, nullptr, 0, d.get(), core);
    d.release();
}

//////////////////////////////////////////
// BlankAudio (turn into alias for WaveGenerator
...
void audioInitialize(VSPlugin *plugin, const VSPLUGINAPI *vspapi) {
    ...
    vspapi->registerFunction("WaveGenerator", "clip:anode:opt;channels:int:opt;bits:int:opt;sampletype:int:opt;samplerate:int:opt;length:int:opt;frequency:int:opt;amplitude:float:opt;waveform:data:opt;", "clip:anode;", waveGeneratorCreate, 0, plugin);
    vspapi->registerFunction("BlankAudio", "clip:anode:opt;channels:int:opt;bits:int:opt;sampletype:int:opt;samplerate:int:opt;length:int:opt;", "clip:anode;", blankAudioCreate, 0, plugin);
}
