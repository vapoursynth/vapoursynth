...
VSMap *readSettings(const std::string &path) {
    VSMap *settings = vs_internal_vsapi.createMap();
    std::string err;
    ...
    if (!f) {
        ...
        if (errno != ENOENT) {
            ...
            vs_internal_vsapi.mapSetError(settings, err.c_str());
        }
        return settings;
    }
        ...
        switch (state) {
            ...
            case WANT_VALUE_CHAR:
                if (buffer[i] == '\n') {
                    ...
                    vs_internal_vsapi.mapSetData(settings, key.c_str(), value.c_str(), value.size(), dtUtf8, paReplace);
