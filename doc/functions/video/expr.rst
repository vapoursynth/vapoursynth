...
.. function:: Expr(vnode[]:all clips, string[] expr[, int format])
   :module: std
   ...
   without the f16c instructions the planes that read or write it are
   processed by the slower interpreter instead of the JIT.
