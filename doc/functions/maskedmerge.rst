...
.. function::   MaskedMerge(clip[] clips, clip mask[, int[] planes, bint first_plane=0])
   ...
   MaskedMerge merges two *clips* using the per pixel weights in the *mask*,
   where 0 means that the original clip is returned unchanged.
   ...
   The clips must have the same dimensions and format and the *mask* must be the
