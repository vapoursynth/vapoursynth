...
.. function:: SelectClip(clip[] clips, clip[] src, func selector)
   ...
   The selector function is passed a dict containing the frame number ('n') and the frame(s) retrieved from *src* are in ('f'), which will be a list if there is more than one source clip.
