...
.. function:: Expr(clip[] clips, data[] expr[, int format])
   ...
   Logical operator are also a bit special since everything is done in floating point arithmetic.
   All values greater than 0 are considered tru for the purpose of comparisons. Logical operators return 0.0 for false and 1.0 for true in their operations.
   Since the expression is being evaluated at runtime there are also the stack manipulation operators, *swap* and *dup* which swaps the topmost and second topmost value and duplicates the topmost stack value respectively.
   ...
   How to average the Y plane of 3 YUV clips and pass through the UV planes unchanged (assuming same format)::
   
      std.Expr(clips=[clipa, clipb, clipb], expr=["x y + z + 3 /", "", ""])
      
   How to average the Y plane of 3 YUV clips and pass through the UV planes unchanged (different formats)::
